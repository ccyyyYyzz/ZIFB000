...
def discover_termes_fonts() -> tuple[str, dict[str, Path], str | None]:
    """Register the manuscript's exact TeX Gyre Termes faces or fail."""
    _directory, found, family = register_termes_fonts(font_manager)
...
def build_derived_tables(
    ...
    trajectories = pd.DataFrame(
        {
            "Q_mAh_cm2": q,
            "R_theta_baseline": a_control / a0_control,
            "R_theta_one_way_island_shadow": a_shadow / a0_shadow,
            "R_theta_coupled_island": a_coupled / a0_coupled,
    ...
    references = pd.DataFrame(
        [
            {
                "reference_id": "baseline_half_accessibility_capacity",
            ...
            {
                "reference_id": "baseline_half_accessibility_solid_fraction",
                "value": float(eps_cal_row["value"]),
                "unit": str(eps_cal_row["unit"]),
                "meaning": "baseline solid-I2 fraction read where R_theta=0.5",
                "source_marker_id": str(eps_cal_row["marker_id"]),
            },
            {
                "reference_id": "island_model_half_accessibility_solid_fraction",
                "value": float(eps_island_row["value"]),
                "unit": str(eps_island_row["unit"]),
                "meaning": "R_theta=0.5 reference of the registered dense island relation",
        ...
        [
            {
                ...
                "Q_half_accessibility_mAh_cm2": float(summary["control"]["Q_theta_0p5_mAh_cm2"]),
                "endpoint_R_theta": float(trajectories["R_theta_baseline"].iloc[-1]),
            ...
            },
            {
                ...
                "Q_s_mAh_cm2": q_s_control,
                "Q_half_accessibility_mAh_cm2": float(summary["one_way_dense_shadow"]["Q_theta_0p5_mAh_cm2"]),
                "endpoint_R_theta": float(trajectories["R_theta_one_way_island_shadow"].iloc[-1]),
                "endpoint_eps_s": float(comparison["eps_s_control"].iloc[-1]),
            ...
            },
            {
                ...
                "Q_half_accessibility_mAh_cm2": np.nan,
                "endpoint_R_theta": float(trajectories["R_theta_coupled_island"].iloc[-1]),
...
def create_figure(
    ...
    a_control = trajectories["R_theta_baseline"].to_numpy(float)
    a_shadow = trajectories["R_theta_one_way_island_shadow"].to_numpy(float)
    a_coupled = trajectories["R_theta_coupled_island"].to_numpy(float)
    ...
    # a — calibration-controlled remaining-accessibility direction.
    ...
    ax_a.set(xlim=(0, 121), ylim=(-0.02, 1.035),
             ...
             ylabel=r"remaining accessibility, $R_\theta$")
    ax_a.set_xticks([0, 40, 80, 120])
    ax_a.set_yticks([0, 0.5, 1.0])
    ax_a.set_title("Calibration-controlled accessibility", loc="left", pad=5)
    style_axis(ax_a)
    ...
    ax_c.scatter([q[-1]], [eps_coupled[-1]], s=18, marker="s", color=GRAPHITE,
                 edgecolor=WHITE, linewidth=0.45, zorder=6, clip_on=False)
    eps_mid = 1000.0 * ref_value("baseline_half_accessibility_solid_fraction")
    eps_island = 1000.0 * ref_value("island_model_half_accessibility_solid_fraction")
    ...
    ax_c.text(3.0, eps_mid + 0.10,
              rf"baseline $R_\theta=0.5$ inventory  {eps_mid:.3f} × 10$^{{-3}}$",
              color=VERMILION, fontsize=6.5, ha="left", va="bottom")
    ax_c.text(3.0, eps_island + 0.10,
              rf"island $R_\theta=0.5$ reference  {eps_island:.3f} × 10$^{{-3}}$",
    ...
    ax_d.annotate(
        "island reference",
        xy=(float(island_point["R_theta_end"]),
    ...
    ax_d.set(xlim=(0, 0.64), ylim=(0, 340),
             xlabel=r"end-of-charge accessibility, $R_{\theta,\mathrm{end}}$",
...
def main() -> None:
    ...
    caption = (
        ...
        "**a,** Calibration-controlled remaining-accessibility factor, $R_\\theta=1-\\theta$, for the calibrated baseline, a one-way "
        ...
        "lines mark the solid-fraction values at $R_\\theta=0.5$ for the baseline and island relation. **d,** "
    ...
    metadata = {
        ...
        "core_conclusion": (
            ...
            "strongly changes later calibration-controlled accessibility, retained-solid inventory, and voltage; "
