...
### Panel a — periodic single-I2 BSSE-corrected adsorption-energy ordering

- Accepted PBE-D3(BJ) electronic adsorption energies plus PBE counterpoise
  BSSE corrections are
...
- Safe inference: the dry periodic calculation supports a relative
  BSSE-corrected adsorption-energy ordering in which the two oxygenated motifs
  are more favorable than basal.
...
### Panel c — single-fibre geometric remaining-area comparators

- Sparse placement uses N = 10^11 m^-2; dense placement uses N = 10^14 m^-2.
- Sparse computed nodes retain geometric A/A0 = 0.890 at
  eps_s = 3.391 x 10^-3.
- Dense computed nodes give geometric A/A0 = 0.544 at
  eps_s = 1.356 x 10^-3 and
  0.293 at eps_s = 4.069 x 10^-3.
- The origin A/A0 = 1 at eps_s = 0 is an exact zero-solid boundary; other
  lines only connect computed nodes and are not a fitted closure.
- Safe inference: assumed placement density strongly changes the geometric
  remaining-area family over the baseline solid-loading range.
- Excluded inference: the curves are not observed morphology, measured
  ...
  $R_{\theta}$ relation. They are not the native COMSOL accessibility
  $A_{\mathrm{bare}}/A_0=R_{\theta}T_{\mathrm{pore}}$, where
...
1. Panel a subtracts the accepted basal BSSE-corrected adsorption energy from
   each accepted site adsorption energy.
...
3. Panel c selects the sparse and dense computed families and adds only the
   exact zero-solid geometric-area boundary. The publication table renames the
   legacy upstream complements as `geometric_A_over_A0` and
   `geometric_removed_fraction` in
   `R582_Fig5_panel_c_geometric_area_families_v2.csv`; their numeric values are
   unchanged. Panel a is likewise versioned as
   `R582_Fig5_panel_a_dft_site_ordering_v2.csv`, leaving the v1 tables intact.
...
The calibrated $R_{\theta}$ relation and native
$A_{\mathrm{bare}}/A_0=R_{\theta}T_{\mathrm{pore}}$ accessibility remain
separate continuum-model objects evaluated in Figure 4.
