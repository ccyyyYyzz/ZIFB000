...
def audit_text(fig, family: str) -> dict:
    ...
    face_charmaps = [
        set(font_manager.get_font(str(path)).get_charmap()) for path in TERMES_PATHS
    ]
    common_charmap = set.intersection(*face_charmaps)
    missing_glyphs = sorted(
        {
            f"U+{ord(character):04X}"
            for item in texts
            for character in item.get_text()
            if ord(character) > 127 and ord(character) not in common_charmap
        }
    )
    ...
    if missing_glyphs:
        raise ValueError(f"TeX Gyre Termes is missing required glyphs: {missing_glyphs}")
    return {
...
def svg_font_audit(path: Path) -> dict:
    ...
    rendered_sizes = [
        float(value)
        for value in re.findall(r"font:[^;\"']*?([0-9]+(?:\.[0-9]+)?)px", text)
    ]
    minimum_rendered_size = min(rendered_sizes) if rendered_sizes else None
    if (
        forbidden
        or "TeX Gyre Termes" not in text
        or minimum_rendered_size is None
        or minimum_rendered_size < 6.5 - 1e-9
    ):
        raise ValueError(
            "SVG font audit failed: "
            f"forbidden={forbidden}, minimum_rendered_size={minimum_rendered_size}"
        )
    return {
        "editable_text": "<text" in text,
        "required_family_present": True,
        "minimum_rendered_text_pt": minimum_rendered_size,
        "forbidden": forbidden,
        "pass": True,
    }
