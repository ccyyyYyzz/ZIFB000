...
does not independently validate the model markers or identify a deposit morphology. The
derivative landmarks did not set $Q_s$ or $Q_{f,\mathrm{cal}}$, but the latter belongs to a
voltage-calibrated branch that uses the shared full-cell trace separately. The ZIFB
