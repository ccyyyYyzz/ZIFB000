...
FONT_DIR, FONT_PATHS, FONT_FAMILY = register_termes_fonts(font_manager)
...
PDFFONTS_EXE = locate_tex_tool("pdffonts", FONT_DIR)
...
def draw_state_relation(ax: plt.Axes) -> None:
    ...
    xs = [0.05, 0.25, 0.46, 0.70, 0.93]
    draw_direct_node(ax, xs[0], "Free-I$_2$\nsupersaturation", r"$S$", "modeled state", COLORS["blue"], 0.10)
    draw_direct_node(ax, xs[1], "Retained\nsolid", r"$\varepsilon_s$", "modeled state", COLORS["amber"], 0.09)
    draw_direct_node(ax, xs[2], "Remaining\naccessibility", r"$R_\theta=1-\theta_{\mathrm{cal}}$", "calibrated relation", COLORS["teal"], 0.15, symbol_size=6.5)
    draw_direct_node(ax, xs[3], "Native reaction\narea", r"$A_{\mathrm{bare}}/A_0=R_\theta T_{\mathrm{pore}}$", "solved area factor", COLORS["teal"], 0.18, symbol_size=6.5)
    draw_direct_node(ax, xs[4], "Voltage\nresponse", r"$V$", "model output", COLORS["graphite"], 0.08)

    verbs = ["gates", "maps", r"$\times\,T_{\mathrm{pore}}$", "modulates"]
    ...
    # The dashed calibration path terminates on R_theta, not on the native area product.
    ax.text(0.58, 0.91, "voltage calibration", ha="center", va="center", fontsize=6.5, fontweight="bold", color=COLORS["vermilion"])
    ax.add_patch(
        FancyArrowPatch(
            (0.58, 0.855),
            (0.50, 0.447),
...
def draw_capacity_rail(ax: plt.Axes, q_s: float, q_f: float) -> None:
    ...
    draw_direct_state(ax, 0.89, r"mean $R_\theta<0.5$", r"after $Q_{f,\mathrm{cal}}$", COLORS["teal"], 0.17, expression_size=6.5)
...
def write_source_table(geometry: dict[str, dict[str, str]], q_s: float, q_f: float) -> Path:
    ...
    rows = [
        ...
        ("b", "remaining accessibility", "R_theta", "1-theta_cal", "fraction", "voltage-calibrated relation", relative(STATE_CSV)),
        ("b", "native reaction-area multiplier", "A_bare/A0", "R_theta*T_pore; T_pore=(K/K0)^0.5", "fraction", "solved COMSOL identity", relative(STATE_CSV)),
...
def run_qa(fig: plt.Figure, outputs: dict[str, Path]) -> tuple[Path, dict[str, object]]:
    ...
    qa_md.write_text(
        ...
        "- Five unboxed causal nodes: PASS\n"
...
def write_manifest(source_table: Path, outputs: dict[str, Path], qa_json: Path, qa: dict[str, object]) -> Path:
    ...
    manifest = {
        "figure": STEM,
        "core_conclusion": "The model separates supersaturation, retained solid, calibrated remaining accessibility, the native reaction-area product, and voltage response in a 2D ZIFB positive electrode.",
        ...
        "causal_node_count": 5,
        "rounded_state_cards": 0,
        "calibration_target": "R_theta = 1 - theta_cal; native A_bare/A0 additionally includes T_pore = (K/K0)^0.5",
